...
def _convert_list_value_to_list_of_TestValue(lst):
    try:
        return list(map(_convert_value_to_TestValue, lst))
    except TypeError:
        return list(map(_convert_value_to_TestValue, [lst]))
...
class InstrumentMetaclass(type):
    ...
    def __new__(cls, class_name, bases, dct):
        attrs = dict((name, value) for name, value in list(dct.items()) if type(value) == type and issubclass(value, GenericCommandClass))
        attrsbis = dict((name, value) for name, value in list(dct.items()) if type(value) == InstrumentMetaclass and issubclass(value, Group))
...
#        out =  dict((name, value) for name, value in dct.items() if not name.startswith('__'))       
        ...
        for (name, value) in list(attrsbis.items()):
            out.update({name: GroupProperty(value)})

        out['_subgroups'] = list(attrsbis.keys())
        out['_property_list'] = list(attrs.keys())
...
if __name__ == "__main__":
    class Generic(object, metaclass=InstrumentMetaclass):
        def __init__(self):
